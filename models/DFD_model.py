...
class DFDModel(BaseModel):
    ...
    def set_input(self, input_data):
        self.imgs, self.mask, self.audios = input_data
    ...
    def forward(self):
        if torch.cuda.is_available():
            self.imgs = torch.autograd.Variable(self.imgs).cuda()
            self.audios = torch.autograd.Variable(self.audios).cuda()
        else:
            self.imgs = torch.autograd.Variable(self.imgs)
            self.audios = torch.autograd.Variable(self.audios)

        self.aud_feat = self.netG_audio(self.audios.squeeze(0))
        self.img_feat  = self.netG_video(self.imgs.squeeze(0))
