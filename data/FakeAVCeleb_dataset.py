...
class FakeAVcelebDataset(BaseDataset):
    ...
    def __init__(self, opt, transform=None):
        ...
        self.video_dir = os.path.join(self.root)
    ...
    def __getitem__(self, index):
        # directory, target, duration = self.clips[index]
        ...
        #     decord_ar = decord.AudioReader(video_name, sample_rate=16000)
        ...
        # return (process_data, mask, audio, target)
        data = self.clips[index]
        return data
    ...
    def _make_dataset(self, directory, setting):
        ...
        clips = []
        file_path = glob.glob(os.path.join(directory, setting, '*.npz'))
        for npz_file in file_path:
            data = np.load(npz_file)
            process_data, mask, audio, target = data['imgs'], data['mask'], data['audio'], data['label']
            item = (process_data, mask, audio, target)
            clips.append(item)
    ...
    def _sample_train_indices(self, num_frames):
        ...
        if average_duration > 0:
            ...
            # 选择视频最后的skip_length帧
            offsets = offsets + num_frames - self.skip_length
    ...
    def _audio_transform(self, decord_ar, frame_id_list, fps):
        ...
        # print(len(decord_ar._array[0]), audio_feature_id_list[0], frame_id_list[0], decord_ar._array[0,-5:])
        # import sys
        # sys.exit(-1)
        ...
        audio = decord_ar._array

        # audio sample according to video
        audio_sample_list = np.array([])
        if self.new_step == 1: # no skip
            audio_sample_list = audio[:,audio_feature_id_list[0]: audio_feature_id_list[-1] + num]
        else:
