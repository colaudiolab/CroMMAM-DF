...
def distortion_vid(vid_in,
    ...
    if dist_type == 'VC':
        ...
        # extract audio
        audio_path = vid_in[:-4] + ".wav"
        cmd = f"ffmpeg -loglevel quiet -i {vid_in} {audio_path} -y"
        os.system(cmd)
        ...
        writer.release()
        if via_xvid:
            cmd = f'ffmpeg -i {vid_out[:-4]}_tmp.avi -i {audio_path} -y {vid_out} -loglevel quiet'
            os.system(cmd)
    if os.path.exists(f'{vid_out[:-4]}_tmp.avi'):
        os.remove(f'{vid_out[:-4]}_tmp.avi')
    if os.path.exists(f'{vid_in[:-4]}.wav'):
        os.remove(f'{vid_in[:-4]}.wav') 
    # print('Finished.')
...
def main(vid_in, vid_out, type, level, via_xvid=True, meta_path=None):
    # args = parse_args()
...
def get_files(directory, setting):
...
if __name__ == '__main__':
    ...
    directory = '/mnt/200ssddata2t/yejianbin/DFDC/processed'
    out_dir = os.path.join(directory, 'perturbation')
    setting = 'test.csv'
    files = get_files(directory, setting)

    with multiprocessing.Pool(processes=10) as pool:
        processes = []
        
        # for typ in ['CS', 'CC', 'BW', 'GNC', 'GB', 'JPEG']:
        for typ in ['GB']:
            for level in ['5']:
            # for level in ['1', '2', '3', '4', '5']:
                for f in tqdm(files):
                    basename = os.path.basename(f)
                    vid_in = os.path.join(directory, f)
                    output_path = os.path.join(out_dir, typ, level, basename)
                    os.makedirs(os.path.dirname(output_path), exist_ok=True)
                    if os.path.exists(output_path):
                        print(output_path)
                        continue

                    # main(vid_in, output_path, typ, level)
                    process = pool.apply_async(main, (vid_in, output_path, typ, level))
